...
log "🌺 Creating Shri Neem Karori Baba Sansthan – Golden Edition …"
...
# ---------- Global CSS (golden header/footer, hamburger, leaf buttons, social cards EXACT) ----------
...
.bars {
  ...
  background-color: #1B0A2A;   /* dark for contrast on golden */
  border-radius: 5px;
  transition-duration: .3s;
}
...
EOF
log "Global CSS with exact Uiverse components written"

# ---------- Root Layout (golden header & footer integrated) ----------
...
# ---------- GOLDEN HEADER (logo center, hamburger left, dropdown) ----------
...
HEADEREOF
log "Golden Header (center logo, hamburger dropdown) created"

# ---------- GOLDEN FOOTER (exact social cards) ----------
...
# ... (remaining sections: ChatWidget, botEngine, pages, horoscope script, placeholders, github actions – all unchanged from previous working version)
# For brevity, I've omitted them here but they are part of the full script.
# The full script would continue with all the CDN-based chatbot, 15 pages, Groq horoscope generator, etc.

# Finalize
git init && git add . && git commit -m "🌺 Golden digital ashram ready"
